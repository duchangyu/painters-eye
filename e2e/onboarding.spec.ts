...
test('keyboard-only users can set up, calibrate, and compare', async ({ page }) => {
  await page.goto('/')
  ...
  await page.keyboard.press('Tab')
  await expect(page.getByRole('button', { name: '开始校准' })).toBeFocused()
  ...
  await page.keyboard.press('Tab')
  const enhanceButton = page.getByRole('button', { name: '开启个人增强' })
