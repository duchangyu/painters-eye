...
test("a validated profile survives reload and skips full calibration", async ({
  ...
  await expect(page.getByRole("button", { name: "开始测试" })).toHaveCount(0);

  await page.getByRole("button", { name: "配置与备份" }).click();
  await expect(page.getByText("当前配置：书房显示器")).toBeVisible();
});

test("a changed display nickname requires the short check", async ({
  page,
}) => {
  await completeFirstRun(page);
  await page.evaluate(() => {
  ...
  await expect(page.getByText("效果短复核 · 8 题")).toBeVisible();
