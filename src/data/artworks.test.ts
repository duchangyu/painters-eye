...
describe("curated artwork metadata", () => {
  it("contains exactly twelve stable, licensed local records", () => {
    expect(ARTWORKS).toHaveLength(12);
    expect(new Set(ARTWORKS.map((artwork) => artwork.id)).size).toBe(12);
    ...
    for (const artwork of ARTWORKS) {
      ...
      expect(artwork.period.trim()).not.toBe("");
