...
describe('GalleryScreen', () => {
  it('presents six local, explained works and opens a selection', async () => {
    const user = userEvent.setup()
    const onSelect = vi.fn()
    render(<GalleryScreen artworks={ARTWORKS} onSelect={onSelect} />)

    expect(screen.getAllByRole('button', { name: /查看/ })).toHaveLength(6)
    expect(screen.getAllByText('为什么选择这幅作品')).toHaveLength(6)
